...
Original file is located at
    https://colab.research.google.com/drive/1BpyufH-aQTVI6B0V22ggK-xOt88Sw8M_
"""

#===============================================
# Código - Port 1 - 01/07 - AL-Quad
#===============================================
...
from docplex.mp.model import Model     # CPLEX/DOcplex para resolver MIQP
import pygmo as pg                     # Métricas multi‑objetivo (hipervolume)
# ===============================================================
# Leitura dos dados do problema Port 1 de OR-LIB
# ===============================================================
...
# ==================================================================================
# Cores usadas em plots de pontos
colors = {1: 'red', 5: 'green', 10: 'orange', 15: 'magenta', 20: 'cyan', 31: 'black'}
...
def Armijo_tamanho_passo(z, d, F, J, A_k, delta):
    ...
    J : Avaliação do gradiente em z na direção d, isto é [GradL_1·d, GradL_1·d].
...
def processo_externo_penalizacao(z0, alpha, max_outer=50, tol_stop=1e-08):
    """
...
nd_points = {};      # "nd_points" lista das frentes não‑dominadas F(z*)=(f_1(z*),f_2(z*))
Mean_time_alpha = {} # Média de tempo externo para cada α
Sum_time_alpha = {}  # Soma dos tempos externos para cada α
...
# Listas temporárias para acumular os resultados de cada start
    ...
    all_ND       [alpha] = ND
    Mean_time_alpha[alpha] = np.mean(times_e)
    Sum_time_alpha[alpha] = np.sum(times_e)
...
# ===================================================
# TABELA 3: Média do tempo para cada alpha
# =====================================================
print("\n α | Tempo Total | Média de Tempo ")
print("-"*50)
for alpha in alpha_values:
    print(f"{alpha:2}| {Sum_time_alpha[alpha]:.4f} | {Mean_time_alpha[alpha]:.4f}")
...
for alpha in alpha_values:
    ...
    for sol in all_solutions[alpha]:
        ...
        ret   = v @ x_f
        plt.scatter(risco, ret,color=colors[alpha],s=20,alpha=0.5,marker='o')
...
for alpha in alpha_values:
    ...
    plt.scatter([],[],color=colors[alpha],s=30,marker='o',label=f'AL-Quad α$=${alpha}')
plt.xlabel('Risco',fontsize=16)
plt.ylabel('Retorno',fontsize=16)
...
alpha_values_miqp = [1,5,20,31]   # Valores de cardinalidade usados
lambdas = np.linspace(0.0, 1, 100) #Valores de pesos na ponderação
...
# =====================================================================================================
# CÁLCULO DE OUTRAS MÉTRICAS DESEMPENHO (NÃO USADO NA PRÁTICA-Tudo está notebook de perfis de desempenho)
...
for alpha in alpha_values:
    # ============= AL-quad =============
    fr_al = [(v @ z[:n], np.sqrt(z[:n] @ (Q @ z[:n])), cardinalidade(z[:n]))
             for z, _ in all_ND[alpha]]
    # ============= MIQP ================
    fr_miqp = [(s['ret'], s['risco'], s['card'])
               for s in pareto2.get(alpha, [])]

    for nome, frente in [("AL-quad", fr_al), ("MIQP", fr_miqp)]:
        if not frente:
            vmin = vmax = qmin = qmax = cmin = cmax = float('nan')
        else:
            rets, risks, cards = zip(*frente)
...
def dykstra(x, y, ro, alfa, up, low, v, maxcycles=500, tol=1e-12):
    ...
    while err >= tol and k < maxcycles: #Roda enquanto o erro for maior que a tolerância e não exceder o número máximo de ciclos
        ...
        x1 = proj_hiperplano(z, e_local, 1.0) #realiza a projeção no hiperplano soma(x)=1, <x,1>=1
        t = x1 - z; #Calcula o deslocamento aplicado pela projeção.
        T = T + np.linalg.norm(t - dk2)**2; #Atualiza erro acumulado (critério de parada) (+= é usado como atalho para T=T+norm, T está acumulando o erro)
        dk2 = t # atualizamos o vetor de correção
        # caixa x
        z = x1 - dk3
        x2 = proj_box(z, low, up)
        t = x2 - z;
        T = T+ np.linalg.norm(t - dk3)**2;
        dk3 = t
        # semiespaço soma(y)>=n-alfa
        z = y - dk5
        y1 = proj_halfspace(z, e_local, n_local - alfa)
        t = y1 - z;
        T = T + np.linalg.norm(t - dk5)**2;
        dk5 = t
        # caixa y
        z = y1 - dk6
        y2 = proj_box(z, low, up)
        t = y2 - z;
        T = T + np.linalg.norm(t - dk6)**2;
        dk6 = t
        err = np.sqrt(T)
        x, y = x2, y2
        k = k + 1
...
def lagrangiano_penalizado(z, tau, lam):
    ...
    x, y = z[:n], z[n:]
    base      = F_mobj(z)                                # [f1, f2]
    lin_term  = np.dot(lam, x * y)                       # lamb(xoy)
...
# ============================================
#  PROJEÇÃO dos pontos brutos de partida
#    raw_z  = ponto bruto (Inviável gerado a partir de z_centro e direção aleatória)
#    z0     = Defino como raw_z
#    proj_z = ponto viável inicial (x,y) por Dykstra usado no multi-start
# ============================================
...
nd_points_exp = {};      # "nd_points" lista das frentes não‑dominadas F(z*)=(f_1(z*),f_2(z*))
Mean_time_alpha = {} # Média de tempo externo para cada
Sum_time_alpha = {}  # Soma dos tempos externos para cada α
...
for alpha in alpha_values:
    for sol,_ in all_ND_exp[alpha]:
        x_f = sol[:n]
        plt.scatter(np.sqrt(x_f@(Q@x_f)), v@x_f, color=colors[alpha], s=30, marker='o')
    plt.scatter([],[],color=colors[alpha],s=30,marker='o',label=f'AL-Exp α$=${alpha}')
plt.xlabel('Risco',fontsize=16)
plt.ylabel('Retorno',fontsize=16)
plt.title('Risco vs Retorno - AL')
plt.legend()
...
for alpha in alpha_values:
    ...
    plt.scatter(arr[:,0], arr[:,1],marker='o',color=colors[alpha],s=60,label=f'AL-Exp α$=${alpha}')
...
for alpha in alpha_values_miqp:
    ...
    plt.scatter(arr[:,0], arr[:,1],
                ...
                s=80, linewidths=1.5,
                label=f'MIQP α$=${alpha}')
plt.title('Risco vs Retorno – AL-Exp vs MIQP')
plt.xlabel('Risco',fontsize=16);  plt.ylabel('Retorno',fontsize=16)
plt.legend();  plt.grid(True);  plt.tight_layout();  plt.show()

# ======================================================================
# CÁLCULO DE OUTRAS MÉTRICAS DESEMPENHO (NÃO FOI USADO NA NOSSA PRÁTICA)
# ======================================================================
#ND_orig = {}
#for α in alpha_values:
...
for alpha in alpha_values:
    ...
    for nome, frente in [("AL-exp", frente_exp), ("MIQP", frente_miqp)]:
        if not frente:
            vmin = vmax = qmin = qmax = cmin = cmax = float('nan')
        else:
            rets, risks, cards = zip(*frente)
