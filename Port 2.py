# -*- coding: utf-8 -*-
"""Port2-30pontos.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/17mcpHb8m99KWa2LwxEOtqkgXUo4prMXS
"""

# ==============================================================================================
# Código 23/06 - AL-Quad - PORT 2
# Este script resolve o problema de otimização de portfólio multi‑objetivo
# para restrição de cardinalidade relaxada com variáveis contínuas, usando o Lagrangeano Aumentado
# com penalidade quadrática (AL‑Quad).
#===============================================================================================
# montagem do Drive
...
#import pygmo as pg                     # Métricas multi‑objetivo (hypervolume)
# ============================================================================
...
#      demais linhas – pares (i,j) + correlação(i,j)
# ===============================================================
url = 'https://people.brunel.ac.uk/~mastjjb/jeb/orlib/files/port2.txt'
...
for linhas in linhas[1 + n:]:   # Fora da diagonal
    ...
    cov_ij = corr * desviopadrao[i] * desviopadrao[j]
    Q[i, j] = Q[j, i] = cov_ij        # Reproduz o simétrico da matriz

v = retorno_esperado
# ==================================================================================
# Não foi usado em prática (Montagem da fronteira irrestrita de Port 1)
#url_f = 'https://people.brunel.ac.uk/~mastjjb/jeb/orlib/files/portef1.txt'
#url_f = "https://people.brunel.ac.uk/~mastjjb/jeb/orlib/files/portef2.txt"
#resp_f = requests.get(url_f); resp_f.raise_for_status()
#lines_f = resp_f.text.splitlines()
#frontier = np.array([list(map(float, ln.split())) for ln in lines_f if ln.strip()])
#ret_f = frontier[:, 0]
#var_f = frontier[:, 1]
#print(f"Número de ativos (n): {n}")
#print(f"Fronteira irrestrita: {len(frontier)} pontos lidos\n")
# ==================================================================================
...
# ============================================
# MÉTODO DE DYKSTRA
# Transcrito para python do trabalho de Krejié (2023)
# ============================================

def dykstra(x, y, ro, alfa, up, low, v, maxcycles=500, tol=1e-12):
    """
    Método de Dykstra para projeções alternadas
    Parâmetros:
    x,y:Vetores de entrada (cada um de tamanho n)
    ro: Não foi usado na prática principal (usado para limitar o retorno)
    alfa: cardinalidade do problema
    up: limite superior da caixa
    low: limite inferior da caixa
    v: não usado na prática (também utilizado para estudar o retorno)
    maxcycles: número máximo de cíclos
    tol: tolerância

    Retorna:
    z: vetor de dimensão 2n na interseão dos conjuntos fechados e convexos (x,y);
    erro
    """
    n_local = len(x) #dimensão do problema.
    e_local = np.ones(n_local) #vetor com todos os elementos iguais a 1 (1,1,1,1,1,...,1)
    dk2 = np.zeros(n_local); dk3 = np.zeros(n_local)#vetores de correção de Dykstra (dk2,...,dk6)
    dk5 = np.zeros(n_local); dk6 = np.zeros(n_local) # um para cada subconjunto convexo e fechado
    err = 2.0; #erro inicial
    k = 0; #contador do ciclo
    while err >= tol and k < maxcycles: #Roda enquanto o erro for maior que a tolerância e não exceder o número máximo de ciclos
        T = 0.0
        # hiperplano soma(x)=1
        z = x - dk2  #remove correção anterior da projeão no hiperplano
        x1 = proj_hiperplano(z, e_local, 1.0) #realiza a projeção no hiperplano soma(x)=1, <x,1>=1
        t = x1 - z; #Calcula o deslocamento aplicado pela projeção.
        T += np.linalg.norm(t - dk2)**2; #Atualiza erro acumulado (critério de parada) (+= é usado como atalho para T=T+norm, T está acumulando o erro)
        dk2 = t # atualizamos o vetor de correção
        # caixa x
        z = x1 - dk3
        x2 = proj_box(z, low, up)
        t = x2 - z;
        T += np.linalg.norm(t - dk3)**2;
        dk3 = t
        # semiespaço soma(y)>=n-alfa
        z = y - dk5
        y1 = proj_halfspace(z, e_local, n_local - alfa)
        t = y1 - z;
        T += np.linalg.norm(t - dk5)**2;
        dk5 = t
        # caixa y
        z = y1 - dk6
        y2 = proj_box(z, low, up)
        t = y2 - z;
        T += np.linalg.norm(t - dk6)**2;
        dk6 = t
        err = np.sqrt(T)
        x, y = x2, y2
        k = k + 1
...
def proj(z, v, low, up, ro, alfa):     # Função para chamar a projeção Dykstra
    x = z[:n]; y = z[n:]
...
def lagrangiano_penalizado(z, tau, lam):
    x, y = z[:n], z[n:]
    base = F_mobj(z)
    # termo: (tau/2)*||xoy + lam/tau||² multiplicado pelo vetor e
    term = (tau / 2) * np.linalg.norm(x * y + lam / tau)**2
    return base + term * np.ones_like(base)

def jacobiano_lagrangiano_penalizado(z, tau, lam):
    """
    ...
    return np.vstack([g1, g2]) # o np.vstack é usado para empilhar arrays como o concatenate, mas os arrays são empilhados verticalmente [[g1_1 g1_2][g2_1 g2_2]]
...
def Armijo_tamanho_passo(z, d, F, J, A_k, delta):
    ...
    J : Avaliação do gradiente em z na direção d, isto é [GradL_1·d, GradL_2·d].
    A_k : Par (A_1, A_2) acumulado nas últimas iterações.
    ...
    """
    for j in range(20):
        t_val = 1/(2**j) #tamanho de passo por backtrancking

        z_next = z + t_val*d #próximo ponto com esse t

        F_next = np.array(F(z_next)) #valor da função, recebe aplicado no lagrangiano aumentado

        right_side = np.array(A_k) + delta*t_val*J #lado direito da condição de Armijo
        if np.all(F_next <= right_side): # verificação componente a componente da condição de Armijo
            return t_val
    return 1
...
def compute_A_k(f_values, k, M):
    ...
    Retorna:
    A_k : Par (A_1, A_2), onde cada componente é o máximo de F_j nas últimas min(k, M) iterações.
    """
    m_k = min(k, M)
...
def processo_interno_PGMMOP(z, tau, lam, alpha, max_iter=50,M=10, delta=0.5, tol_inner=1e-08):
    ...
    for k in range(max_iter):
        ...
        f_vals.append(Fzp) #adiciona Fzp na lista de valores da função trabalhada.
        d0 = np.zeros_like(z) #vetor de zeros com mesma dimensão de z.
      ...
      # Definimos os vetores gradientes em relação a x e y da função lagrangiana:
        ...
        res = minimize(obj_suave, dt0, method='SLSQP',
                      jac=obj_suave_grad,
                      constraints=cons,
                      options={'ftol':1e-04, 'maxiter':500, 'disp':False})

        # Recupera a direção ótima d (descarta o valor ótimo t)
        d = res.x[:-1]
        Ak = compute_A_k(f_vals, k, M)
        Jv = np.array([
            jacobiano_lagrangiano_penalizado(z,tau,lam)[0]@d,
            jacobiano_lagrangiano_penalizado(z,tau,lam)[1]@d])
...
def processo_externo_penalizacao(z0, alpha, max_iter=50, tol_stop=1e-8):
    """
    Executa o loop externo de atualização de tau e lamb para o Lagrangiano aumentado.
    ...
    return z, d, max_iter, Fz, tau, norm_diff, lam, t_ext, t_int

# ====================================================
# Parâmetros do multi-start
# ====================================================
n_starts     = 30
alpha_values = [1,2,3,4,5,6,7,8,10,15,20,25,30,35,40,45,50,55,60,65,70,75,80,82,85]
#25 alfas
# ============================================
# GERAÇÃO DOS PONTOS INICIAIS
# ============================================
np.random.seed(42)      # torna as listas reprodutíveis, semente de reprodução

unif_inits = []         # lista de pares de vetores z=(x,y) unificando x e y

for _ in range(n_starts):
    x_rand = np.random.uniform(0.0, 1.0, n)
    y_rand = np.random.uniform(0.0, 1.0, n)
    z0_rand = np.concatenate([x_rand, y_rand])
    unif_inits.append(z0_rand) #pares de pontos gerados aleatoriamente
...
all_tau = {};        # "all_tau" lista de valores finais de \tau
...
nd_points = {};      # "nd_points" lista das frentes não‑dominadas F(z*)=(f1(z*),f2(z*))
...
for alpha in alpha_values:
    ...
    proj_inits = []   # lista para guardar os pontos após a projeção inicial
    raw_points  = []  # lista para guardar os pontos inviáveis iniciais (brutos)

    for raw_z in unif_inits:
        z0 = raw_z.copy()
        proj_z = proj(z0, v, low=0, up=1, ro=0, alfa=alpha)
        raw_points.append(z0)        # guarda o ponto inviável
        proj_inits.append(proj_z)   # guarda o projetado

    # ===========================================================
    # MULTI-START: roda o AL+PGM a partir de cada proj_z
    # ===========================================================
    for i, (z0_raw, z0) in enumerate(zip(raw_points, proj_inits), 1): #"zip(raw_points, proj_inits)" combina os elementos das duas listas em pares;
        print(f"\n--- Início {i} para α = {alpha} ---")               # "enumerate(zip(raw_points, proj_inits), 1)" enumera os pares começando em i=1 até i=N (número de starts)


        (z_opt, d, it_ex, Fz, tau_f,
         diff_f, lam_f, t_ext, t_int) = processo_externo_penalizacao(z0, alpha)

        # armazena resultados nas listas
        sols .append(z_opt)
        Fs   .append(Fz)
        taus .append(tau_f)
        diffs.append(diff_f)
        ...
        print("Ponto bruto (antes proj.):", np.round(z0_raw, 4))
        ...
        # ===========================================================
        # Calculamos os objetivos originais (f1,f2) para o ponto ótimo atual z_opt
        ...
        for zp, Fp in ND:  # Percorre todos os pontos da lista ND (não dominados existentes) e verifica se z_opt é dominado por algum deles
            # Fp <= F_orig é um vetor booleano (True ou False) com o resultado dessa comparação em todos os componentes de f1 e f2.
            ...
            # and liga essas duas questões necessárias para a definição clássica de dominância de Pareto.
        ...
        # Só executamos se z_opt não for dominado.
        if not dominado:
            new_ND = []  # lista auxiliar para armazenar os pontos que permanecem não dominados
            for zp, Fp in ND:  # percorre todos os pontos já em ND
                # Se F_orig domina Fp (ou seja, F_orig <= Fp em todos e < em pelo menos uma componente),
                # então descartamos esse ponto. Caso contrário, mantemos na lista.
                if not (np.all(F_orig <= Fp) and np.any(F_orig < Fp)):
                    new_ND.append((zp, Fp))  # Se zp não é dominado por F_orig, mantemos zp na lista
            # Se z_opt não for dominado, adicionamos ele na lista de não dominados
            new_ND.append((z_opt, F_orig))
            # Atualiza a lista de não-dominados
            ND = new_ND
        # Caso z_opt seja dominado, ND permanece inalterado
    ...
    # calcula fronteira não-dominada de α
    front = np.array([
    [0.5 * (sol[:n] @ (Q @ sol[:n])),   # 0.5xQx
     -(v @ sol[:n])]                    # –retorno
    for sol, _ in ND])
...
# ================================================================
# MIQP PONDERADO via CPLEX/DOcplex para diferentes alphas
#(fonte:https://ibmdecisionoptimization.github.io/docplex-doc/)
# ================================================================

alpha_values_miqp = [1,2,3,4,5,6,7,8,10,15,20,25,30,35,40,45,50,55,60,65,70,75,80,82,85]
lambdas = np.linspace(0.0, 1, 30) #Valores de pesos na ponderação
all_sols2 = []  #lista vazia para guardar as soluções do MIQP CPLEX
...
def solve_weighted(alpha, lam):
    ...
    f1 =  0.5 * x^T Q x (variância)
    f2 = -v^T x (objetivo de retorno negativo)
    risco = sqrt(x^T Q x)
    ret = v^T x
...
df_p.columns = ['α','λ','f1','f2','Card','Ret','Risco','Tempo(s)']
print("\n--- Soluções não-dominadas (Pareto) MIQP ---")
print(df_p.to_string(index=False, float_format="%.4f"))

# neste ponto você já tem 'pareto2' com cada sol['time']
time_miqp = sum(s['time'] for lst in pareto2.values() for s in lst)

# ============================================
# CÁLCULO DE OUTRAS MÉTRICAS DESEMPENHO (NÃO USADAS NA NOSSA PRÁTICA)
# Loop sobre cada α para calcular HV, Delta - spread, t_min
# ============================================
# Constroinovamente ND_orig: não-dominados do problema original
#ND_orig = {}
#for alf in alpha_values:
    # calcula todos os objetivos originais (f1,f2) = (variância, -retorno)
#    objs = [F_mobj(z) for z in all_solutions[alf]]
#    nd = []
#    for o in objs:
...
#for α in alpha_values:
    # monta não-dominados de (half_var,-retorno)
#    final_objs = np.array(ND_orig[α])
    # ponto de referência
#    ref_global = final_objs.max(axis=0) + 1e-1 #Usado apenas nessas métricas
...
#out_path = f'/content/drive/MyDrive/meus_experimentoscom50starts2/{out_name}'
#np.savez(out_path,
...
# ================================
# Código 23/06 - AL-Exp - PORT 2
# Este script resolve o problema de otimização de portfólio multi‑objetivo
# para restrição de cardinalidade relaxada com variáveis contínuas, usando o Lagrangeano Aumentado
# com penalidade exponencial (AL‑Exp).
#=================================
# montagem do Drive
from google.colab import drive  # Permite gravar e ler arquivos no Google Drive
drive.mount('/content/drive')# cria o drive em /content/drive  – só usado no Google Colab
...
def lagrangiano_penalizado(z, tau, lam):
    """
    L(x,y,lamb,tau) = F_mobj(x)+[lamb^T (xoy)+ tau*sum(e^{(x_i y_i)^2} - 1)
                               +||lamb||²/(2tau)]·e,
    onde e = (1, 1)^T.
    ...
    for k in range(max_iter):
      ...
      # Definimos os vetores gradientes em relação a x e y da função lagrangiana:
        ...
        Jv = np.array([
            jacobiano_lagrangiano_penalizado(z,tau,lam)[0]@d,
            jacobiano_lagrangiano_penalizado(z,tau,lam)[1]@d])
        t = Armijo_tamanho_passo(z, d, lambda zz: lagrangiano_penalizado(zz, tau, lam),
                                      Jv, Ak, delta)
        z_new = z + t*d
        z_new = proj(z_new, v, 0, 1, 0, alpha)
        if np.linalg.norm(d) <= tol_inner:
...
all_solutions = {};  # "all_solutions" lista dos vetores ótimos z* encontrados para cada start
all_F = {};          # "all_F" lista dos valores de F(z*) (o vetor de objetivos)
all_tau = {};        # "all_tau" lista de valores finais de tau
all_diff = {};       # "all_diff" lista de valores ΔF (mudança de F)
all_iter = {};       # "all_iter" lista de números de iterações externas usadas
all_time = {};       # "all_time" lista de tempos (s) gastos em cada run
all_ND = {};         # "all_ND" lista de listas de soluções não‑dominadas encontradas
...
for alpha in alpha_values:
    ...
    for raw_z in unif_inits:
        z0 = raw_z.copy()
        proj_z = proj(z0, v, low=0, up=1, ro=0, alfa=alpha)
        raw_inits.append(z0)        # guarda o bruto
        proj_inits.append(proj_z)   # guarda o projetado

    # ===========================================================
    # MULTI-START: roda o AL+PGMMOP a partir de cada proj_z
    # ===========================================================
    for i, (z0_raw, z0) in enumerate(zip(raw_inits, proj_inits), 1):
        ...
        # ================================================
        # Atualiza lista de não-dominados
        # ================================================
    ...
    # calcula Γ-spread para fronteira não-dominada de α
    front = np.array([
    [0.5 * (sol[:n] @ (Q @ sol[:n])),   # 0.5 x^TQx
     -(v @ sol[:n])]                    # –retorno
    for sol, _ in ND])
    nd_points [alpha] = front.copy()
